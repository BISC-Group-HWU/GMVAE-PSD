...
class SSVAE:
    ...
    def create_dataset(self, is_training, data, labels, batch_size, x_labeled=None, y_labeled=None):
      ...
      if labels is None:
        ...
        # at each iter, we take 10 = (batch_size - batch_size_labeled) new data
        # and we concat 40 = (batch_size_labeled) labeled data to have a full
        # batch size = 50

      iterator_init = iterator.initializer

      if labeled_data:
        output = {'data': _data, 'labels': _labels,
    ...
    def create_model(self, is_training, inputs, output_size):
      ...
      out_logits, y_mean, y_var, output = self.network.decoder(
          gaussian, categorical, output_size, is_training)

      # reconstruction loss
      if self.loss_type == 'bce':
        loss_rec = self.losses.binary_cross_entropy_gmvae(data, out_logits)
      ...
      # kl-divergence loss
      # loss_kl = self.losses.kl_gaussian(mean, var)
      loss_kl = self.losses.labeled_loss(gaussian, mean, var, y_mean, y_var)

      # loss_kl_cat = self.losses.kl_categorical(prob, log_prob, self.num_classes)
      loss_kl_cat = -self.losses.entropy(_logits, prob)
      ...
      if _labels is not None:
        ...
        if labeled_ss is not None:
          ...
          predicted_labels = assign_labels_semisupervised(gaussian, labeled_ss, self.num_labeled_batch,
          ...
          # change HERE
          # loss_assign = tf.losses.sparse_softmax_cross_entropy(labels=_labels, logits=_logits)
        ...
        if self.metric_loss == 'triplet':
          ...
          loss_metric = tf.contrib.losses.metric_learning.triplet_semihard_loss(predicted_labels, gaussian, margin=self.metric_margin)
        elif self.metric_loss == 'lifted':
          # loss_metric = tf.constant(0.)
          loss_metric = tf.contrib.losses.metric_learning.lifted_struct_loss(predicted_labels, gaussian, margin=self.metric_margin)
        else:
      ...
      # total loss
      loss_total = loss_vae + self.w_assign * \
    ...
    def evaluate_dataset(self, is_training, num_batches, model_spec, labeled_data=None, labeled_labels=None):
      ...
      if is_training:
        ...
        for j in range(num_batches):
          ...
          # change HERE
          _,_,_x_labeled, _y_labeled, _, _ = create_semisupervised_dataset_training(labeled_data, labeled_labels,
                                                                       self.num_classes, self.num_labeled_batch)

          # run the tensorflow flow graph
          _, loss_rec, loss_kl, loss_metric, loss_assign, loss_cat, loss_total, true_labels, predicted_labels, accuracy=self.sess.run([train_optimizer,
                                                     ...
                                                     model_spec['true_labels'], model_spec['predicted_labels'], model_spec['accuracy']],
        ...
        for j in range(num_batches):
          # run the tensorflow flow graph
          loss_rec, loss_kl, loss_metric, loss_assign, loss_cat, loss_total, true_labels, predicted_labels, accuracy = self.sess.run([ 
                                                     model_spec['loss_rec'], model_spec['loss_kl'], 
                                                     model_spec['loss_metric'], model_spec['loss_assign'], 
                                                     model_spec['loss_kl_cat'], model_spec['loss_total'],
                                                     model_spec['true_labels'], model_spec['predicted_labels'], model_spec['accuracy']],
    ...
    def train(self, train_data, train_labels, val_data, val_labels, labeled_data, labeled_labels,load_model,save_dir):
      """Train the model
      ...
      # change HERE
      # train_dataset = self.create_dataset(True, train_data, train_labels, self.batch_size)
      train_dataset = self.create_dataset(True, train_data, train_labels, self.batch_size - self.num_labeled_batch, self.x_labeled, self.y_labeled)
    ...
    def test(self, test_data, test_labels, batch_size = -1):
      ...
      # evaluate the model
      list_logits = np.empty((0, self.num_classes))
      list_predicted_labels = []
      list_true_labels = []
      ...
      avg_accuracy = 0.0
      for j in range(num_batches):
        _accuracy, _logits, _predicted_labels, _true_labels = self.sess.run([accuracy, logits, predicted_labels, true_labels], 
                                          feed_dict={self.network.temperature: self.temperature
                                                     ,self.w_metric: self._w_metric
                                                     ,self.learning_rate: self.lr})
        avg_accuracy += _accuracy
        list_logits = np.append(list_logits, _logits, axis=0)
      ...
      avg_accuracy /= num_batches
      nmi = self.metrics.nmi(predicted_labels, true_labels)
      accuracy = self.metrics.cluster_acc(predicted_labels, true_labels)

      logits, predicted_labels = np.array(logits), np.array(predicted_labels)
      return logits, predicted_labels, accuracy, nmi, avg_accuracy
    ...
    def latent_features_mean(self, data, batch_size=-1):
      # if batch_size is not specified then use all data
      if batch_size == -1:
        batch_size = data.shape[0]
