...
def create_semisupervised_dataset(x, y, num_classes, num_labeled, seed_i=0):
    """
    ...
    ind_labeled = np.empty((0))
    ind_labeled = ind_labeled.astype('int32')

    ind_unlabeled = np.empty((0))
    ind_unlabeled = ind_unlabeled.astype('int32')
    ...
    for i in range(num_classes):
        ...
        ind_labeled = np.append(ind_labeled,indices[:labeled_per_class],axis=0) 
        ind_unlabeled = np.append(ind_unlabeled,indices[labeled_per_class:],axis=0)  
 
    return ind_labeled, ind_unlabeled, np.vstack(x_labeled), np.hstack(y_labeled), np.vstack(x_unlabeled), np.hstack(y_unlabeled)  



def create_semisupervised_dataset_training(x, y, num_classes, num_labeled):
    if num_labeled % num_classes != 0:
      raise "wished number of labeled samples not divisible by number of classes"
