...
class Networks:
    ...
    def __init__(self, params):
      if params is not None:
        ...
        self.num_neurons_big = params.num_neurons_big
        self.num_neurons_small = params.num_neurons_small
    ...
    def encoder_fc(self, input_data, num_classes, is_training=False):
      ...
      with tf.variable_scope('encoder_fc', reuse=not is_training):
        ...
        out = tf.layers.dense(out, units=296)
        out = tf.nn.relu(out)
        out = tf.layers.dense(out, units=self.num_neurons_big)
        out = tf.nn.relu(out)
        out = tf.layers.dense(out, units=self.num_neurons_small)
        out = tf.nn.relu(out)
        ...
        # concat = tf.layers.dense(concat, units=self.num_neurons_big)
        # concat = tf.nn.relu(concat)
        # concat = tf.layers.dense(concat, units=self.num_neurons_small)
        # concat = tf.nn.relu(concat)
        concat = tf.concat([categorical, out], 1)
    ...
    def output_from_gaussian(self, gaussian, output_size, is_training=False):    
      ...
      with tf.variable_scope('decoder_gauss', reuse=not is_training):                
        ...
        out = tf.layers.dense(gaussian, units=self.num_neurons_small)
        out = tf.nn.relu(out)
        out = tf.layers.dense(out, units=self.num_neurons_big)
        out = tf.nn.relu(out)
    ...
    def decoder_fc(self, gaussian, categorical, output_size, is_training=False):
      ...
      # gaussian2 = self.latent_gaussian(categorical, self.gaussian_size)
      # out, reconstructed = self.output_from_gaussian(gaussian2['gaussian'], output_size, is_training)
        
      return out, mean, var, reconstructed
      # return out, gaussian2['mean'], gaussian2['var'], reconstructed
